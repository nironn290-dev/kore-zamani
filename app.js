...
// Her film için benzersiz key: baslik + yil kombinasyonu
function itemKey(item) {
  return (item.baslik + '_' + item.yil).replace(/\s+/g, '_').toLowerCase();
}

function isSaved(item) { return watchList.some(w => itemKey(w) === itemKey(item)); }
...
function renderResults(list, q) {
  ...
  list.forEach((item, i) => {
    ...
    const starsHtml = [1,2,3,4,5].map(s =>
      '<span class="star ' + (rating >= s ? 'lit' : '') + '" data-idx="' + i + '" data-val="' + s + '">★</span>'
    ).join('');
    ...
    html += '<div class="r-card">' +
      ...
      '<div class="social-row">' +
        '<button class="like-btn ' + (liked ? 'liked' : '') + '" id="like-' + sid + '" data-idx="' + i + '">' +
          '<svg viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>' +
          '<span id="lc-' + sid + '">' + likeCount + '</span>' +
        '</button>' +
        '<button class="yorum-toggle" data-sid="' + sid + '" data-idx="' + i + '">Yorumlar</button>' +
      '</div>' +
      '<div id="yorumlar-' + sid + '" style="display:none;">' +
        '<div class="yorum-area">' +
          ...
          '<div class="yorum-inp-row">' +
            ...
            '<button class="yorum-send" data-sid="' + sid + '" data-idx="' + i + '">Gönder</button>' +
          '</div>' +
        '</div>' +
      '</div>' +
      '<div class="r-actions">' +
        '<button class="save-btn ' + (saved ? 'saved' : '') + '" id="sb-' + sid + '" data-idx="' + i + '">' + (saved ? '♥ Listede' : '+ Listeye Ekle') + '</button>' +
...
function attachEvents() {
  // Yıldız — her film kendi index'ine göre
  ...
  document.querySelectorAll('.save-btn').forEach(el => {
    el.addEventListener('click', function() {
      ...
      const item = currentResults[idx];
      if (!item) return;
      const existIdx = watchList.findIndex(w => itemKey(w) === itemKey(item));
